...
parmgrid <- parmgrid0[45:nrow(parmgrid0),]
...
#   beepr::beep(2) ## play a sound so you know it's ready (pokemon!!!)
# } ## end RepE
# proc.time() - p
...
foreach(RepE = unique(parmgrid[, "EM"]),
  ...
  run_assessments(RootFile, Species = Species, parmgrid = parmgrid, RepE = RepE, SSPlots = T)
stopCluster(cl)
proc.time() - p
